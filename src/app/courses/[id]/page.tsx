    ...
    <div className="my-6 rounded-xl overflow-hidden border border-gray-200 shadow-lg">
      <div className="bg-gray-800 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-xs font-semibold px-2 py-1 rounded ${getLanguageColor(
              language
            )} text-white`}
          >
...
export default function CourseViewerPage() {
    ...
    // Declare pollCourse with useRef to avoid circular dependency
    const pollCourseRef = useRef<(() => Promise<void>) | null>(null);
  
    useEffect(() => {
      const pollCourse = async () => {
        try {
          const response = await courseAPI.get(Number(params.id));
          const courseInfo = response.data;
          setCourse(courseInfo);
  
          if (courseInfo.status === 'completed') {
            const contentResponse = await courseAPI.getContent(Number(params.id));
            const data = contentResponse.data.content as CourseData;
            ...
            if (data.index?.modules?.length > 0) {
              ...
              if (firstModule.chapters?.length > 0) {
                ...
                setExpandedChapters(
                  new Set([`${firstModule.moduleId}-${firstChapter.chapterId}`])
                );
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-gray-900">{course.topic}</h1>
              <p className="text-sm text-gray-600 capitalize">
                {course.level} • Completed{' '}
                {course.completed_at && formatDateTime(course.completed_at)}
              </p>
            </div>
          </div>
        </div>
...
function TopicViewer({ topic }: { topic: TopicContent }) {
    ...
    const videos = Array.isArray(topic.resources?.videos)
      ...
      parts.forEach((part) => {
        if (part.type === 'code') {
          ...
          lines.forEach((line, idx) => {
            ...
            // Detect list items (starts with *, -, or bullet points)
            if (trimmed.match(/^[\*\-•]\s+/)) {
              if (currentParagraph) {
              ...
              // Check if next line is also a list item or end of lines
              const nextLine = lines[idx + 1]?.trim();
              if (!nextLine?.match(/^[\*\-•]\s+/) && listItems.length > 0) {
    ...
    // Helper function to format inline text with bold/italic
    const formatInlineText = (text: string) => {
      return text
        // Bold text with **text** or *text*: at start
        .replace(/\*\*([^*]+)\*\*/g, '<strong class="font-bold text-gray-900">$1</strong>')
        // Italic text or terms followed by colon (like "Supervised Learning:")
        .replace(/([A-Z][a-zA-Z\s]+):/g, '<strong class="font-bold text-indigo-700">$1:</strong>')
    ...
    return (
      <article className="max-w-5xl mx-auto p-8 bg-white">
        ...
        {/* Additional Sections from sections array */}
        {sections.length > 0 && (
          <section className="mb-12">
...
// Fix 2: Change VideoCard type to be more explicit
function VideoCard({
    video,
  }: {
    video: {
